/**
 * CLEAN METAPLEX UMI VERSION: Attach metadata to SDM token
...
## 🔧 **Steps for your GCP VM:**

1. **Install the correct UMI dependencies:**
   ```bash
   npm install @metaplex-foundation/umi-bundle-defaults @metaplex-foundation/mpl-token-metadata @metaplex-foundation/umi
   ```

2. **Replace your current file** with the clean code above (make sure there are no `</invoke>` tags or other XML-like content)

3. **Update your mint address** in the script:
   ```typescript
   const MINT_ADDRESS = 'YOUR_ACTUAL_MINT_ADDRESS_HERE';
   ```

4. **Run the script:**
   ```bash
   npx ts-node attach_metadata_umi.ts
   ```

## 🆕 **Key Changes for UMI Framework:**
- ✅ Uses `createUmi()` instead of `Metaplex.make()`
- ✅ Uses UMI signers and public key types
- ✅ No more `bundlrStorage` (handled automatically)
- ✅ Modern Metaplex API patterns

This should resolve all your TypeScript compilation errors! 🎯